...
@connect((state) => ({
  rules: state.Attendance.Reports.rules,
  modules: state.General.modules,
  meta: state.Attendance.Reports.meta,
}), {
  index,
  emptyRules,
})
...
/**
 *
 * @class ReportsListContainerWrapper
 * @extends PureComponent
 */
export default class ReportsListContainerWrapper extends React.PureComponent {
  static propTypes = {
  ...
  _reload(page = 0, limit = app.config.pagination.limit) {
    ...
    let _modules = modules.find((item) => item.name === 'Report');
    emptyRules();
    index({
      includes: ['type'],
      filterGroups: [
  ...
  _dataSource() {
    ...
    app._.map(rules, (rule) => {
      rule.icon = 'file-chart';
      rule.label = (rule.type ? rule.type.label : undefined );
    });
    return rules;
  }
