...
export default class ListContainer extends React.PureComponent {
  ...
  _renderMenu(item) {
    ...
    return (
      ...
      >
        <Menu.Item key="edit" disabled={!can('PostCategory@update')}>
          <MaterialIcon name="pencil"/>
          {app.translate('main.Edit')}
        </Menu.Item>
        <Menu.Item key="delete" disabled={!can('PostCategory@destroy')}>
    ...
    return (
      <Button.Group>
        <Button
          type="dashed"
          onClick={onReload}
          disabled={!can('PostCategory@index') || loading}
        >
        ...
        <Button
          type="primary"
          icon="plus"
          disabled={!can('PostCategory@store') || activeItem || statusForm === 'editing'}
          onClick={onAdd}
        >
