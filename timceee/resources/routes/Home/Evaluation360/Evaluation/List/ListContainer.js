...
export default class ListContainer extends React.PureComponent {
  ...
  _renderMenu(item) {
    ...
    return (
      ...
      >
        <Menu.Item key="edit" disabled={!can('Setting@update')}>
          <MaterialIcon name="pencil"/>
          {app.translate('main.Edit')}
        </Menu.Item>
        <Menu.Item key="delete" disabled={!can('Setting@destroy')}>
    ...
    return (
      <Button.Group>
        <Button
          type="dashed"
          onClick={onReload}
          disabled={!can('Setting@index') || loading}
        >
        ...
        <Button
          type="primary"
          icon="plus"
          disabled={!can('Setting@store') || activeItem || statusForm === 'editing'}
          onClick={onAdd}
        >
