...
export default class ListContainer extends React.PureComponent {
  ...
  _renderMenu(item) {
    ...
    return (
      ...
      >
        <Menu.Item key="edit" disabled={!can('Period@update')}>
          <MaterialIcon name="pencil"/>
          {app.translate('main.Edit')}
        </Menu.Item>
        <Menu.Item key="delete" disabled={!can('Period@destroy')}>
    ...
    return (
      <Button.Group>
        <Button
          type="dashed"
          onClick={onReload}
          disabled={!can('Period@index') || loading}
        >
        ...
        <Button
          type="primary"
          icon="plus"
          disabled={!can('Period@store') || activeItem || statusForm === 'editing'}
          onClick={onAdd}
        >
