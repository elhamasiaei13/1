...
export default class RequestsTable extends React.PureComponent {
  ...
  render() {
    const {
      ...
      onChangeTable, onSelectAllTable, params, get,
      onSelectTable,
    } = this.props;
    ...
    const columns = [
      {
        ...
        render: (text, record) => {
          let _return = '';
          _return += record.request && record.request.senderPosition && record.request.senderPosition.name ? record.request.senderPosition.name : '';
          _return += record.request &&
          record.request.senderUser &&
          record.request.senderUser.profile ?
            ` (${record.request.senderUser.profile.firstName} ${record.request.senderUser.profile.lastName})` : '';
          return _return;
        },
      },
      {
        ...
        render: (text, record) => {
          ...
          let _requestType = this.findObject(requestTypes, {id: record.request.typeId});
      ...
      },
      {
        ...
        key: 'description',
        dateTimeFilter: 'range',
        render: (text, record) => record.request && record.request.description,
      },
